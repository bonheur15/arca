...
export function AdminPoliciesPage() {
  ...
  return (
    <AdminLayout title="User policies" description="Set quota and capability boundaries per person.">
      {users.isPending ? <LoadingState compact label="Loading users…" /> : users.isError ? <ErrorState error={users.error} /> : !users.data.length ? <EmptyState title="No users to configure" description="Create a member before assigning a policy." /> : <><div className="policy-picker"><Field label="Policy for"><select onChange={(event) => setUserId(event.target.value)} value={userId}>{users.data.map((user) => <option key={user.id} value={user.id}>{user.displayName || user.username} · @{user.username}</option>)}</select></Field></div>{current.isPending ? <LoadingState label="Loading policy…" /> : current.isError ? <ErrorState error={current.error} onRetry={() => void current.refetch()} /> : <div className="policy-grid"><section className="settings-section"><div className="settings-section__head"><h2>Capacity</h2><p>Quota includes active content, versions, and trash.</p></div><div className="settings-form"><div className="toggle-list"><div><div><strong>Unlimited storage</strong><p>Ignore the personal quota; host reserve still applies.</p></div><button aria-checked={policy.unlimited} className="switch" onClick={() => toggle("unlimited")} role="switch" type="button"><span /></button></div></div><div className="form-grid"><Field label="Quota"><div className="input-suffix"><input disabled={policy.unlimited} min={1} onChange={(event) => setPolicy({ ...policy, quotaBytes: Number(event.target.value) * 1024 ** 3 })} type="number" value={Math.round(policy.quotaBytes / 1024 ** 3)} /><span>GB</span></div></Field><Field label="Max item count"><input min={1} onChange={(event) => setPolicy({ ...policy, maxItems: Number(event.target.value) })} type="number" value={policy.maxItems} /></Field><Field label="Max single file"><div className="input-suffix"><input min={0} onChange={(event) => setPolicy({ ...policy, maxFileBytes: Number(event.target.value) * 1024 ** 2 || null })} placeholder="Unlimited" type="number" value={policy.maxFileBytes ? Math.round(policy.maxFileBytes / 1024 ** 2) : ""} /><span>MB</span></div></Field><Field label="Blocked extensions" hint="Comma-separated; leave empty to allow all"><input onChange={(event) => setPolicy({ ...policy, blockedExtensions: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })} placeholder="exe, dmg" value={policy.blockedExtensions.join(", ")} /></Field><Field label="Allowed MIME groups" hint="Comma-separated top-level groups; leave empty to allow all"><input onChange={(event) => setPolicy({ ...policy, allowedMimeGroups: event.target.value.split(",").map((item) => item.trim().toLowerCase()).filter(Boolean) })} placeholder="image, audio, application" value={policy.allowedMimeGroups.join(", ")} /></Field></div></div></section><section className="settings-section"><div className="settings-section__head"><h2>Capabilities</h2><p>Control outward access and upload pressure.</p></div><div className="toggle-list">{([['allowInternalSharing', 'Internal sharing', 'Share with existing members.'], ['allowPublicSharing', 'Public sharing', 'Create short-lived five-digit codes.'], ['allowApiTokens', 'Developer tokens', 'Use the external API.']] as const).map(([key, label, hint]) => <div key={key}><div><strong>{label}</strong><p>{hint}</p></div><button aria-checked={policy[key]} className="switch" onClick={() => toggle(key)} role="switch" type="button"><span /></button></div>)}</div><div className="form-grid"><Field label="Concurrent uploads"><input max={20} min={1} onChange={(event) => setPolicy({ ...policy, maxConcurrentUploads: Number(event.target.value) })} type="number" value={policy.maxConcurrentUploads} /></Field><Field label="Pending uploads"><input max={100} min={1} onChange={(event) => setPolicy({ ...policy, maxPendingUploads: Number(event.target.value) })} type="number" value={policy.maxPendingUploads} /></Field><Field label="Active public shares"><input max={1000} min={0} onChange={(event) => setPolicy({ ...policy, maxActivePublicShares: Number(event.target.value) })} type="number" value={policy.maxActivePublicShares} /></Field><Field label="Max public lifetime"><div className="input-suffix"><input max={30} min={1} onChange={(event) => setPolicy({ ...policy, maxPublicTtlMinutes: Number(event.target.value) })} type="number" value={policy.maxPublicTtlMinutes} /><span>min</span></div></Field><Field label="Upload rate limit" hint="Leave empty for unlimited"><div className="input-suffix"><input min={0} onChange={(event) => setPolicy({ ...policy, uploadRateBytes: Number(event.target.value) * 1024 ** 2 || null })} placeholder="Unlimited" type="number" value={policy.uploadRateBytes ? policy.uploadRateBytes / 1024 ** 2 : ""} /><span>MB/s</span></div></Field><Field label="Download rate limit" hint="Leave empty for unlimited"><div className="input-suffix"><input min={0} onChange={(event) => setPolicy({ ...policy, downloadRateBytes: Number(event.target.value) * 1024 ** 2 || null })} placeholder="Unlimited" type="number" value={policy.downloadRateBytes ? policy.downloadRateBytes / 1024 ** 2 : ""} /><span>MB/s</span></div></Field></div></section></div>}{save.error ? <p className="form-error">{describeError(save.error)}</p> : null}<div className="settings-actions"><Button disabled={save.isPending || current.isPending} onClick={() => save.mutate()}>{save.isPending ? "Saving…" : "Save policy"}</Button></div></>}
    </AdminLayout>
  );
}
